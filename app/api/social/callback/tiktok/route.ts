...
export async function GET(req: Request) {
  ...
  try {
    ...
    const mockPlatformId = `tt_${Math.random().toString(36).substring(7)}`;
    const mockName = `TikTok User ${Math.floor(Math.random() * 100)}`;
    ...
    await prisma.activityLog.create({
      data: {
        ...
        details: `Connected TikTok account: ${mockName}`
      }
    });
